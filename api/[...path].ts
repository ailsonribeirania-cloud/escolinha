...
export default async function endpoint(req:IncomingMessage & {body?:unknown},res:ServerResponse){
 ...
 let body:string|undefined;if(!['GET','HEAD'].includes(req.method??'GET')){const chunks:Buffer[]=[];let size=0;const limit=req.url?.startsWith('/api/child-photo')?4000000:20000;for await(const chunk of req){size+=chunk.length;if(size>limit){res.writeHead(413,{'Content-Type':'application/json'});res.end(JSON.stringify({message:'Solicitação muito grande.'}));return;}chunks.push(Buffer.from(chunk));}body=Buffer.concat(chunks).toString('utf8');}
