...
function initialTheme():Theme{const value=localStorage.getItem('escolinha-theme');return value==='light'||value==='dark'||value==='system'?value:'system';}
function resolvedTheme(theme:Theme){return theme==='system'?(window.matchMedia('(prefers-color-scheme: dark)').matches?'dark':'light'):theme;}
...
export function AppProvider({children}:{children:ReactNode}) {
 ...
 useEffect(()=>{const apply=()=>{document.documentElement.dataset.theme=resolvedTheme(theme);};apply();localStorage.setItem('escolinha-theme',theme);if(theme!=='system')return;const media=window.matchMedia('(prefers-color-scheme: dark)');media.addEventListener('change',apply);return()=>media.removeEventListener('change',apply);},[theme]);
